...
function create_signature({ path, timestamp, method, body = '' }) {
    ...
    return crypto.createHmac('sha256', base64_key).update(data).digest('base64')
}
...
async function get_btc_balance() {
    if (
        !process.env.COINBASE_EXCHANGE_API_KEY ||
        !process.env.COINBASE_EXCHANGE_API_SECRET ||
        !process.env.COINBASE_EXCHANGE_API_PASSPHRASE
    ) {
        throw new Error(
            'COINBASE_EXCHANGE_API_KEY, COINBASE_EXCHANGE_API_SECRET, and COINBASE_EXCHANGE_API_PASSPHRASE must be set'
        )
    }
    if (!BTC_ACCOUNT_ID) {
