...
function CreateAuthenticationSignature(apiPrivateKey, apiPath, endPointName, nonce, apiPostBodyData) {
...
async function QueryPrivateEndpoint(endPointName, inputParameters) {
    ...
    const signature = CreateAuthenticationSignature(apiPrivateKey, privatePath, endPointName, nonce, apiPostBodyData)
    const httpOptions = {
        headers: { 'API-Key': apiPublicKey, 'API-Sign': signature },
        responseType: 'application/json',
    }
    const { data } = await axios.post(apiEndpointFullURL, apiPostBodyData, httpOptions).catch((err) => {
        console.log(err)
        return {}
    })
...
async function withdraw({ amount_btc }) {
    ...
    try {
        const data = {
            ...
            amount: (Math.floor(amount_btc * 1e8) / 1e8).toFixed(8),
        }
        console.log(data)
